---
title: What is Jetski? | MCP Server Hosting Platform
description: Learn about Jetski, the serverless platform for deploying MCP servers. One-command deploy, Claude/ChatGPT integration, OAuth2 auth & analytics.
...
Jetski is an open-source, serverless hosting, authentication and analytics provider to deploy MCP servers with a single command.
Engineers use Jetski to let their users easily use their MCP servers by browser-based MCP clients, like Claude, ChatGPT and improve the authorization experience by using the latest OAuth2 login standard for MCP servers out of the box.
...
graph TB
    ...
    %% Developer Tools
    Developer[👨‍💻 Developer] --> CLI[⚡ Jetski CLI]
    Developer --> DeveloperBrowser[🌐 Browser]

    CLI --> |Deploy MCP server| Platform[🏗️ Jetski Platform & 📊 Analytics Engine]
    DeveloperBrowser --> |Manage MCP server| Platform


    %% Core Platform Components
    UserBrowser <--> |OAuth2 Flow| AuthServer[🔐 Authentication Server]
    Proxy[🔄 Jetski Proxy] --> |Collects merics| Platform
...
<Aside type="caution">
  Jetski is currently in early access. Join our [waitlist](/waitlist) to get priority access and be among the first to deploy MCP servers with ease.
</Aside>
